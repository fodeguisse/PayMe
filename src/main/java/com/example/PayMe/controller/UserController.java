...
public class UserController {
	...
	public String getDarkAdmin(Model model, RedirectAttributes ra){
		User thief = userService.getConnectedUser();
		model.addAttribute("thief", thief);
		model.addAttribute("preys", userService.getAllUser());
		return "darkAdmin";
	}

	@PostMapping("/darkadmin")
	public String stealingPrey(Model model, RedirectAttributes ra, @RequestParam(value="myprey") String mmyprey){
		User thief = userService.getConnectedUser();
		User user = userService.getUserByName(mmyprey);
		if(user!=null) {
			user.stealingMoneyPrey();

		}
		thief.moneySteal();
		userService.save(thief);
